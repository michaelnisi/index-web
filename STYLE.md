...
The most common form. A blockquote followed by an attribution line, nothing else.

```markdown
# The Quiet Activism of Living Pleasantly

> Because think of the amazing people that you see every day. …

– Jon Stewart to David Remnick at the [New Yorker festival](https://…)
```

### Quote With Commentary

A blockquote followed by one or two sentences of context or response. The commentary names the author in prose and carries the link.

```markdown
Lorin Hochstein relaxes the AI conversation by [adding perspective](https://…).
```

Keep it short. The quote does the work; the commentary frames it, often with a light twist: "Fittingly, the warning reads like it could have been written by a model."

### Essay

Longer pieces (300–850 words) built on a single idea.
...
Code can be the whole post. A fenced block with a language tag, followed by a single sentence explaining why it matters, with a link to the source.
...
- **One `#` heading per post**, the title. Essays run without subheadings.
- **Links go inline on meaningful words**—"[talk]", "[adding perspective]", a publication name—not on "click here." A bare "[here]" is acceptable in a short commentary line.
...
- In-prose attribution: name the person, their role if helpful, and link the source: "Mustafa Suleyman, the British AI entrepreneur who runs Microsoft AI, [cautions](https://…) against…"
